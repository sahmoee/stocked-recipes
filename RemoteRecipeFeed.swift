...
// Refresh interval is read from feed_config.json (set by the Recipe Manager) instead of a
// hardcoded 6 hours. Apply to the Stocked repo (replaces the existing RemoteRecipeFeed.swift).
import Foundation
...
enum RemoteRecipeFeed {
    ...
    static func fetch() async -> [OnlineRecipe] {
        ...
        await APIResponseCache.shared.store(cleaned, for: cacheKey, ttl: await refreshTTL())
        return cleaned
    }
}
